...
This module implements functionality to generate latex documents
...
class LatexDocument(object):
    ...
    def compile( self ):
        ...
        self.tex_file.close()
        output = subprocess.Popen(["pdflatex", '-interaction=batchmode', self.tex_name], stdout=subprocess.PIPE).communicate()[0]
        print(output)
...
def subparser_pdfbook(subparsers):
    """Subparser for *loki pdfbook*"""
    parser = subparsers.add_parser("pdfbook", help="compile images into pdf book", description="Compile images into PDF book")
    parser.add_argument("figures", nargs="+", metavar="IMAGE", help="Input IMAGE files")
    parser.add_argument( "-x", dest="nx", type=int, metavar="NX", default = 2,
            help="Horizontal number of plots" )
    parser.add_argument( "-y", dest="ny", type=int, metavar="NY", default = 3,
            help="Horizontal number of plots" )
    parser.add_argument( "-c", "--caption", dest="caption", action="store_true", default = False,
            help="Include filename in caption" )
    parser.add_argument( "-o", "--output", dest="output", metavar="FILENAME", default = 'pdfbook.tex',
            help="Name of the TeX pdfbook output file" )
    parser.add_argument( "-t", "--title", dest="title", metavar="TITLE",
            help="Create Title Page with TITLE" )
    parser.add_argument( "-f", "--titlefig", dest="titlefig", metavar="IMAGE",
            help="Specify IMAGE for title page" )
    parser.set_defaults(command=command_pdfbook)

#______________________________________________________________________________=buf=
def command_pdfbook(args):
    """Subcommand for *loki pdfbook*"""
    gallery = LatexGallery(
        args.output, 
        args.figures, 
        _nx = args.nx, 
        _ny = args.ny, 
        _captions = args.caption,
        _title = args.title,
        _title_fig = args.titlefig,
        ) 
    gallery.execute()
